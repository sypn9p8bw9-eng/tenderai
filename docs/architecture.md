...
The standalone embedding worker selects a provider through `EMBEDDING_PROVIDER`, which defaults to `openai`. In production it must remain `openai`: the worker calls OpenAI only from a trusted Node process, requires `OPENAI_API_KEY`, and retains the configurable `EMBEDDING_MODEL`. `OPENAI_API_KEY` and `SUPABASE_SERVICE_ROLE_KEY` are server-only; neither is imported by client components or exposed through `NEXT_PUBLIC_` variables. Every stored vector remains tagged with the model that produced it.

M7.2 also provides `mock-local-1536` solely to exercise local worker, persistence, and retrieval plumbing without provider cost. It deterministically hashes text into normalized 1,536-dimensional vectors, does not call OpenAI, and does not require an OpenAI key. These vectors have no semantic validity: their similarity rankings must not be interpreted as retrieval quality or used for any product conclusion. Configuration accepts mock mode only with `NODE_ENV=development` or `NODE_ENV=test` and rejects alternate model names in mock mode, preventing accidental production use or mixing with OpenAI vectors.
